...
const EditListForm = ({ singleList }) => {
  const navigate = useNavigate();
  const { editId } = useParams();
  useEffect(() => {
    if (singleList && singleList.userRef !== auth.currentUser.uid) {
      toast.error('لا تستطيع تعديل الصفحة');
      navigate('/');
    }
  }, []);
  const [formData, setFormData] = useState({
    ...
    beds: singleList.beds,
    paths: singleList.paths,
    parking: singleList.parking,
    furnished: singleList.furnished,
    address: singleList.address,
    description: singleList.description,
    offer: singleList.offer,
    price: singleList.price,
    discount: singleList.discount,
    images: singleList.images,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    const docRef = await updateDoc(doc(db, 'listings', editId), formDataCopy);
    setLoading(false);
    toast.success('تم تعديل اعلانك بنجاح');
  ...
  return (
    <section className='my-4 px-2'>
      <h1 className=' text-dark text-center  mb-3'>تعديل قائمة</h1>
